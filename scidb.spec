...
%bcond_with insource
%define scidb               scidb-15.7
%define scidb_boost         scidb-15.7-libboost
%define scidb_boost_version 1.54.0-0
%define scidb_mpich2        scidb-15.7-mpich2
%define scidb_cityhash      scidb-15.7-cityhash
...
for p in %{scidb} %{scidb}-dbg %{scidb}-client %{scidb}-client-dbg %{scidb}-utils %{scidb}-utils-dbg %{scidb}-dev-tools %{scidb}-dev-tools-dbg %{scidb}-shim %{scidb}-plugins %{scidb}-plugins-dbg %{scidb}-client-python %{scidb}-client-python-dbg %{scidb}-dev %{scidb}-jdbc; do
    ...
    %if %{without insource}
        utils/licensing.pl %{buildroot}/${scidb_install_prefix} utils/scidb.lic
    %else
    	${SCIDB_SOURCE_PATH}/utils/licensing.pl %{buildroot}/${scidb_install_prefix} ${SCIDB_SOURCE_PATH}/utils/scidb.lic
    %endif
done
...
mkdir -p /opt/scidb/15.7/etc/alternatives
mkdir -p /opt/scidb/15.7/lib/alternatives
%{_sbindir}/update-alternatives \
  --altdir /opt/scidb/15.7/etc/alternatives \
  --admindir /opt/scidb/15.7/lib/alternatives \
  --install /opt/scidb/15.7/lib/scidb/plugins/libdense_linear_algebra.so \
  dense_linear_algebra.15.7 /opt/scidb/15.7/lib/scidb/plugins/libdense_linear_algebra-scidb.so 10
%{_sbindir}/update-alternatives \
  --altdir /opt/scidb/15.7/etc/alternatives \
  --admindir /opt/scidb/15.7/lib/alternatives \
  --install /opt/scidb/15.7/lib/scidb/plugins/liblinear_algebra.so \
  linear_algebra.15.7 /opt/scidb/15.7/lib/scidb/plugins/liblinear_algebra-scidb.so 10
...
if [ $1 -eq 0 ] ; then
  ...
  --altdir /opt/scidb/15.7/etc/alternatives \
  --admindir /opt/scidb/15.7/lib/alternatives \
  --remove dense_linear_algebra.15.7 /opt/scidb/15.7/lib/scidb/plugins/libdense_linear_algebra-scidb.so
  %{_sbindir}/update-alternatives \
  --altdir /opt/scidb/15.7/etc/alternatives \
  --admindir /opt/scidb/15.7/lib/alternatives \
  --remove linear_algebra.15.7 /opt/scidb/15.7/lib/scidb/plugins/liblinear_algebra-scidb.so
fi
